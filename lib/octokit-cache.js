...
function scheduleSaveEntries() {
  if (!_needSaveEntries) {
    ...
    _needSaveEntries = setTimeout(saveEntries, 10000); // wait 10s
  }
}

function flush() {
  if (_needSaveEntries) {
    clearTimeout(_needSaveEntries);
    saveEntries();
  }
}
...
octokit.get = async function(query_url, options) {
  let params = Object.assign({}, options); // make a copy
  if (!ENTRIES) {
    await init();
  }

  // -1 forces a total refresh of the cache
  ...
  if (entry && ttl != -1) {
    ...
    if (ttl && isExpired(entry.time, ttl)) {
