...
octokit.get = async function(req, res, query_url, options) {
  res.serverTiming.set('github', 0);
  ...
  const ttl = (req.ttl !== undefined) ? req.ttl : config.ttl;
  ...
  async function gh_get(url) {
    ...
    while (url) {
      ...
      res.serverTiming.set('github', Date.now() - start);
      if (!response.headers) {
        throw response;
      }
      if (!etag) {
    ...
    const rate = response.headers['x-ratelimit-remaining'];
    if (rate) {
      res.set('X-Ratelimit-Remaining', rate);
    ...
    // write the daa on disk, without blocking
