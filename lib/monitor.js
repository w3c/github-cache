...
class Measure {
  ...
  terminate() {
    ...
    add_timing(this.name, this.end - this.start);
  }
}
...
class ServerTiming {
  ...
  getMeasure(name, description) {
    ...
    this._serverTiming.push(measure);
    return measure;
  }
  getHeader() {
    let header = `r;dur=${formatTiming(performance.now() - this.startTime)}`;
    let order = 1;
    if (this._serverTiming.length < 50) {
      this._serverTiming.forEach(measure => {
        measure.terminate();
...
exports.install = (app, options) => {
  ...
  app.get(path + "/usage", (req, res, next) => {
    ...
    cache.request("GET /rate_limit")
      ...
      .then(data => {
        obj.GitHub = data;
        obj.GitHub.minimumRemaining = cache.getMinimumRemaining();
        res.json(obj);
        next();
      });
  });
