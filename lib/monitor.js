...
class ServerTiming {
  ...
  set(name, description) {
    const dur = performance.now() - this._startTime;
    ...
    if (value) {
      ...
      this._serverTiming.set(name, value);
    }
    add_timing(name, dur);
  }
  get(name) {
  ...
  forEach(fct) {
    fct("responseEnd", performance.now() - this._startTime);
      ...
      if (data.referer && data.traceId && data.resources && Array.isArray(data.resources)) {
        for (const rt of data.resources) {
          ...
          if (rt.serverTiming && rt.serverTiming.length > 0) {
            const requestLength = rt.responseStart - rt.requestStart;
            const sts = {};
            for (const st of rt.serverTiming) {
              sts[st.name] = st.duration;
            }
            if (requestLength > 0 && sts.responseEnd && (requestLength - sts.responseEnd) > 0) {
              const start = rt.requestStart + ((requestLength - sts.responseEnd) / 2);
              rt.serverRequestStart = start;
              rt.serverResponseEnd = start + sts.responseEnd;
              rt.serverGitHubStart = (sts.gitHubStart) ? start + sts.gitHubStart : 0;
              rt.serverGitHubEnd = (sts.gitHubEnd) ? start + sts.gitHubEnd : 0;
              rt.serverReadStart = (sts.readStart) ? start + sts.readStart : 0;
              rt.serverReadEnd = (sts.readEnd) ? start + sts.readEnd : 0;
            }
          }
          beacon_add(rt);
        }
