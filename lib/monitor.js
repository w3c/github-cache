...
exports.install = (app, options) => {
  ...
  app.use((req, res, next) => {
    ...
    res.sendServerTiming = () => {
      let serverTiming = `cache;dur=${Date.now() - res.startTime}`;
      res.serverTiming.forEach((key, value, desc) => {
        serverTiming += `, ${key};dur=${value}` + ((desc) ? `;desc="${desc}"` : "");
      });
  ...
  app.post(path + "/beacon", require("body-parser").text(), (req, res, next) => {
    ...
    try {
      ...
      (Array.isArray(data)) ? data.forEach(process) : process(data);
      res.status(200).send();
    } catch (error) {
      exports.error(error);
