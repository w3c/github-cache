...
function scheduleSaveEntries(log) {
  if (!_needSaveEntries) {
    // there is no need to always save cache.json every single time
    _needSaveEntries = setTimeout(function () {
      saveEntries();
      if (log === true) monitor.log("cache was saved");
    }, 60000); // wait 60s before writing
  }
}
...
CACHE.getCacheStatus = async () => {
  ...
  data.entryStatus.undefined = 0;
  for (const [key, entry] of Object.entries(ENTRIES)) {
    // count the number of entries
    data.entries++;
    // count the number of entries per status code
    if (!entry.status) {
      data.entryStatus.undefined++;
    } else {
      const cc = data.entryStatus[entry.status] || 0;
      data.entryStatus[entry.status] = cc + 1;
    }
  }
  return data;
}
...
async function reloadEntrybyKey(key) {
  // console.log(`[INVALID] ${entry.num} ${key} ${entry.status} ${file.status} ${file.message}`);
  if (!ENTRIES) {
    await init();
  }
  const entry = ENTRIES[key];
  try {
    await CACHE.get({ ttl: -1 }, undefined, key, {});
  } catch (error) {
    if (!(error instanceof CacheStatus)) {
      throw error; // abort and catch fire
    }
  }
  file = await readCacheEntry(entry);
  if (file instanceof CacheStatus) {
    console.error(`[ERROR] ${entry.num} ${key} ${entry.status} ${file.status} ${file.message}`);
    if (file.status === 507) {
      if (file.status !== entry.status) {
    ...
    console.log(`[RESTORED] ${entry.num} ${key}`);
  }
}

async function checkEntryByKey(key) {
  if (!ENTRIES) {
    await init();
  }
  if (REMAINING <= 500) { // 500 just in case we have pending requests...
    throw new Error("REACHED GITHUB LIMIT"); // abort and catch fire
  }
  const entry = ENTRIES[key];
  if (!entry.status || entry.status < 400) {
    let file = await readCacheEntry(entry);
    if (file instanceof CacheStatus) {
      error_found_count++;
      reloadEntryByKey(key);
    } else {
      console.log(`${entry.num} ${key}`);
    }
  } else {
    console.log(`${entry.num} ${key} ${entry.status}`);
  }

}

CACHE.checkEntries = async () => {
  if (!ENTRIES) {
    await init();
  }

  let i = 0;
  const limits = await CACHE.getGitHubRateLimit();
  console.log(`CACHE.checkEntries`);
  console.log(`${MAX} entries to check`);
  REMAINING = limits.rate.remaining;
  console.log(`Github rate limit: ${REMAINING}`);
  console.log("--------------------------");
  ...
  scheduleSaveEntries(true);
  console.log("--------------------------");
  console.log(`${MAX} entries checked`);
  console.log(` - ${error_found_count} cache errors`);
  console.log(` - ${error_count} errors left`);
  if (error_count > 0) {
    console.log(errors);
  }
}

// Fix cache entries, depending on http error code
CACHE.fixEntries = async () => {
  if (!ENTRIES) {
    await init();
  }
  ...
  scheduleSaveEntries(true);
  monitor.log(` CACHE.fixEntries found ${error_count} errors`);
  return await CACHE.getCacheStatus();
}
