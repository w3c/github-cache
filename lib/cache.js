...
octokit.get = async (request, response, query_url, params, merge_data, cached_data, entry, ifModifiedSince, currentEntryTime) => {
  ...
  while (url) {
    if (REMAINING <= 500) { // 500 just in case we have pending requests...
      resetRemaining();
      return new CacheStatus(503, `GitHub only has ${REMAINING} requests remaining. Try again in one hour or so.`);
    }
    try {
    ...
    REMAINING = Number.parseInt(resp.response.headers['x-ratelimit-remaining']);
    if (REMAINING < MIN_REMAINING) {
      MIN_REMAINING = REMAINING;
    }
    monitor.gh_log(`${resp.status} ${url} [${REMAINING}]`);
    if (Number.isNaN(REMAINING)) {
      // this shouldn't not happen but it does?!?
      monitor.error("Got a NaN for remaining github requests?\n" + JSON.stringify(resp.response.headers));
    }
    if (measure) {
      measure.stop();
    }
    ...
    if (!etag) {
      etag = resp.response.headers.etag;
    }
    ...
    url = ((resp.response.headers.link || "").match(
      /<([^>]+)>;\s*rel="next"/
    ) || [])[1];
  }
  if (response && !response.compounded) {
    const rate = resp.response.headers['x-ratelimit-remaining'];
    if (rate) {
      response.set('X-Ratelimit-Remaining', rate);
      response.set('X-Ratelimit-Reset', resp.response.headers['x-ratelimit-reset']);
    }
  }
