...
const cache_dir = path.resolve(path.resolve(__dirname, ".."), config.cache);

console.log(`Cache is located in ${cache_dir}`);

/* read a JSON file and return an object, using Promise
   Used directly readCacheEntry and init
   */
const readFile = (filename) => {
  return fs.readFile(path.resolve(cache_dir, filename)).then(JSON.parse);
};
...
const mkdir = (param_path) => {
  let subpath = path.resolve(config.cache, param_path);
  if (DIRS[subpath]) {
...
const writeFile = (filename, obj) => {
  return fs.writeFile(path.resolve(cache_dir, filename), JSON.stringify(obj));
};
...
async function init() {
  ...
  _init = fs.mkdir(cache_dir).catch(err => err).then(() => {
    return readFile("cache.json")
      .then(data => {
        ...
        monitor.warn(`Initialize ${cache_dir}/cache.json`);
...
octokit.get = async (request, response, query_url, params, merge_data, cached_data, entry, ifModifiedSince, currentEntryTime) => {
  ...
  while (url) {
    if (REMAINING <= 500) { // 500 just in case we have pending requests...
      resetRemaining();
      return new CacheStatus(503, `GitHub only has ${REMAINING}. Try again in one hour or so.`);
    }
    try {
...
CACHE.get = async function(request, response, query_url, params) {
  ...
  const githubRequest = REQUEST_PENDING[query_url] =
    octokit.get(request, response,
      ...
      if (data === undefined) {
        monitor.error(`Improper value from octokit.get ${query_url}`);
