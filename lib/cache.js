  ...
  for (const [key, entry] of Object.entries(ENTRIES)) {
    ...
    if (entry.status != 404) {
      ...
      if (file instanceof CacheStatus) {
        error_found_count++;
        console.log(`[INVALID] ${entry.num} ${key} ${file.message}`);
        config.ttl = -1; // forced invalidation
        try {
          await CACHE.get(undefined, undefined, key, {});
        } catch (error) {
          if (!(error instanceof CacheStatus)) {
            saveEntries(); // save the cache state
            throw error; // abort and catch fire
          }
        }
        config.ttl = old_ttl; // restore config.ttl
